...
struct PerfRecord
{
    ...
    PerfRecord() { }
    PerfRecord(const char *theName, bool start) : mName(theName), mStart(start) { mTime = SDL_GetPerformanceCounter(); }
};
...
static inline void InsertPerfRecord(PerfRecord &theRecord)
{
    ...
    else
    {
        ...
        if(anItr != gPerfInfoSet.end())
        {
            ...
            {
                int64_t aDurationTicks = theRecord.mTime - anItr->mStartTime;
                anItr->mDuration += aDurationTicks;

                int64_t aFreq = SDL_GetPerformanceFrequency();
                double aDurationMs = (double)(aDurationTicks * 1000.0) / aFreq;

                if (aDurationMs > anItr->mLongestCall)
                    anItr->mLongestCall = aDurationMs;

                anItr->mMillisecondDuration = (double)(anItr->mDuration * 1000.0) / aFreq;
            }
        }
    }
}
...
{
    int64_t aTime1 = SDL_GetPerformanceCounter();

    for(int i=0; i<gPerfRecordTop; i++)
        InsertPerfRecord(gPerfRecordVector[i]);

    gPerfRecordTop = 0;
    int64_t aTime2 = SDL_GetPerformanceCounter();
