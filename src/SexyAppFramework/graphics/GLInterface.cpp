...
extern int gMinTextureWidth;
extern int gMinTextureHeight;
extern int gMaxTextureWidth;
extern int gMaxTextureHeight;
extern DWORD gSupportedPixelFormats;
extern bool gTextureSizeMustBePow2;
extern const int MAX_TEXTURE_SIZE;
extern bool gLinearFilter;
...
static void CopyImageToTexture(MemoryImage *theImage, int offx, int offy, int texWidth, int texHeight, PixelFormat theFormat, bool create)
{
	...
	{
		// 使用统一的RGBA格式，让stb_image自动处理格式转换
		uint32_t *aDest = new uint32_t[texWidth * texHeight];
		...
		if (bottomPad)
		{
			uint32_t *dstrow = aDest + (texWidth*aHeight);
			memcpy(dstrow, dstrow-(texWidth*4), (texWidth*4));
		}

		if (create)
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, texWidth, texHeight, 0, GL_BGRA, GL_UNSIGNED_INT_8_8_8_8_REV, aDest);
		else
			glTexSubImage2D(GL_TEXTURE_2D, 0, offx, offy, texWidth, texHeight, GL_BGRA, GL_UNSIGNED_INT_8_8_8_8_REV, aDest);
...
{
//	theImageFlags = ImageFlag_MinimizeNumSubdivisions;
	...
	static std::vector<int> aGoodTextureSize;
	static bool haveInited = false;
	if (!haveInited)
	{
		haveInited = true;
		aGoodTextureSize.resize(MAX_TEXTURE_SIZE);
		int i;
		int aPow2 = 1;
